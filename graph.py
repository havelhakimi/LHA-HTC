...
class GraphLayer(nn.Module):
    def __init__(self, config, last=False):
        ...
        if GRAPH == 'GRAPHORMER':
            ...
            self.hir_attn =None # GCNConv(config.hidden_size, config.hidden_size)
        elif GRAPH == 'GAT':
            self.hir_attn = None # GATConv(config.hidden_size, config.hidden_size, 1)
    ...
    def forward(self, label_emb, extra_attn, self_attn_mask, inputs_embeds, cross_attn_mask):
        if GRAPH == 'GRAPHORMER':
            ...
            label_emb = None #self.hir_attn(label_emb.squeeze(0), edge_index=extra_attn)
        if self.last:
            label_emb=label_emb.expand(inputs_embeds.size(0), -1, -1)
            label_emb_text = self.cross_attn(inputs_embeds, label_emb,
                                        attention_mask=cross_attn_mask.unsqueeze(1), output_attentions=True,
                                        only_attn=True)
            label_emb=label_emb[0,:,:]
            return label_emb,label_emb_text
    ...
    def forward(self, inputs_embeds, attention_mask, labels, embeddings):
        ...
        if self.graph:
            if GRAPH == 'GRAPHORMER':
                ...
                extra_attn = None #self.edge_list
        for hir_layer in self.hir_layers:
            label_emb,label_emb_text = hir_layer(label_emb, extra_attn, self_attn_mask, inputs_embeds, cross_attn_mask)

        
        token_probs = label_emb_text.mean(dim=1).view(attention_mask.size(0), attention_mask.size(1),
        ...
        return label_emb,contrast_mask
